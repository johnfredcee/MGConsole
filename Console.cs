...
namespace MGConsole
{
	class Console : IDisposable
	{
		...
		public void LoadContent(GraphicsDevice device, GraphicsDeviceManager deviceManager, InputManager inputManager,
		...
		{
			_graphicsDeviceManager.PreparingDeviceSettings -= OnPreparingDeviceChanged;
			System.Console.SetOut(_systemOut);
			SpriteBatch?.Dispose();
		...
		private void UpdateTimer(float deltaSeconds, ConsoleState stateToSetWhenFinished)
		{
			...
			if (_transitionTimer.Finished)
			{
				...
				{
					_inputManager.PushInputContext("ConsoleAction");
				}
			}
		}
	...
	internal class ConsoleSettings
	{
		...
		public float HeightRatio { get; set; } = 0.9f;
